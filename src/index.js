...
/*
const listItem = document.getElementsByClassName("links");
const dropdownItem = document.getElementsByTagName("option");
...
dropdownAll.addEventListener("click", () => {
    ...
    while (dogBreed.firstChild) {
        dogBreed.firstChild.remove();
    }
    if (dropdown.value == firstLetters) {
        const dogBreed = document.getElementById("dog-breeds");
        const result = listItemArray.filter(listItemArray => firstLetters != dropdown.value);

        dogBreed.appendChild(result);

            const dogBreed = document.getElementById("dog-breeds");
            dogBreed.style.display = "none";
            const filteredItem = document.createElement("li");
            filteredItem.textContent = listItem[i].textContent;
            filteredItem.className = "links";
            dogBreed.appendChild(filteredItem);
